...
url<-"http://srkvcoe.org/"
links<-getHTMLLinks(url)
print(links)

i=2
while (i<length(links)) {
  links1<-getHTMLLinks(links[i])
  print(paste(i,",",links1))
  i=i+1
}
...
try(getHTMLExternalFiles(("http://www.omegahat.net")))


try(getHTMLExternalFiles("http://www.geos.ed.ac.uk/~weather/jcmb_ws/"))
try(getHTMLLinks("http://www.geos.ed.ac.uk/~weather/jcmb_ws/"))
# }
url<-getHTMLLinks("http://www.geos.ed.ac.uk/~weather/jcmb_ws/")
url<-getHTMLLinks("https://www.geos.ed.ac.uk/~weather/jcmb_ws/")
url<-getHTMLLinks("https://www.geos.ed.ac.uk/~weather/jcmb_ws/")
url<-getHTMLExternalFiles("http://www.omegahat.net")
links <- getHTMLLinks(url)
print(links)
...
filenames <- links[str_detect(links, "stat")]
print(filenames)
